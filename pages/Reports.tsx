...
const REPORT_LIST: ReportDefinition[] = [
  // 1. CAIXA & FLUXO
  { id: 'CASH_FLOW_DAILY', category: 'CASH', title: 'Fluxo de Caixa Diário (Analítico)', description: 'Detalhamento cronológico com saldo acumulado real.', orientation: 'landscape' },
  { id: 'CASH_BY_ROUTE', category: 'CASH', title: 'Rentabilidade por Linha', description: 'Consolidado de receitas, passageiros e despesas por rota operacional.', orientation: 'portrait' },
  
  // 2. FINANCEIRO GERENCIAL
  { id: 'DRE_SYNTHETIC', category: 'FINANCIAL', title: 'D.R.E. Gerencial (Competência)', description: 'Demonstrativo de Resultado. Receita vs Despesa (Independente do pagamento).', orientation: 'portrait' },
...
const PrintableReport = React.forwardRef<HTMLDivElement, { 
    ...
    return (
        <div ref={ref} className="print-container bg-white p-8 min-h-screen text-slate-900 font-sans">
                    ...
                    <thead>
                        <tr className="bg-slate-100 border-y-2 border-slate-300 text-slate-700 text-xs uppercase tracking-wider print:bg-slate-100">
                            {data.headers.map((h, i) => (
                                <th key={i} className={`py-2 px-2 font-bold ${i > 0 ? 'text-right' : 'text-left'} ${i === 1 ? 'w-2/5' : ''}`}>{h}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200">
                        {data.rows.length > 0 ? (
                            data.rows.map((row, rIdx) => {
                                const isTotalRow = String(row[0]).includes('TOTAL') || String(row[0]).includes('SALDO') || String(row[0]).includes('MÉDIA');
                                return (
                                    <tr key={rIdx} className={`break-inside-avoid ${isTotalRow ? 'bg-slate-100 font-bold border-t-2 border-slate-300' : 'even:bg-slate-50'}`}>
                                        {row.map((cell, cIdx) => (
                                            <td key={cIdx} className={`py-1.5 px-2 ${cIdx > 0 ? 'text-right' : 'text-left'} text-xs text-slate-700 break-words whitespace-normal`}>
                                                {cell}
                                            </td>
                                        ))}
                                    </tr>
            ...
            {/* Footer Note */}
            {data.footerNote && (
                <div className="mt-8 text-xs text-slate-500 italic border-l-2 border-slate-300 pl-3 bg-slate-50 p-2 break-inside-avoid">
...
export const ReportsPage: React.FC = () => {
  ...
  useEffect(() => {
    setData({
        ...
        commissions: storage.getCommissions()
    });
  }, []);
  ...
  const reportOutput = useMemo((): ReportData => {
    ...
    if (selectedReportId === 'CASH_FLOW_DAILY') {
        ...
        // 1. CALCULATE INITIAL BALANCE (Previous to Start Date)
        const getPreviousBalance = () => {
            let bal = 0;
            // Inflows
            data.routes.filter((r:any) => r.date < dateRange.start).forEach((r:any) => bal += r.cashHanded);
            data.agencies.filter((a:any) => a.date < dateRange.start).forEach((a:any) => bal += a.valueReceived);
            data.tourism.filter((t:any) => t.receivedValue && t.departureDate < dateRange.start).forEach((t:any) => bal += t.receivedValue);
            // Outflows (Expenses PAID before start date)
            data.expenses.filter((e:any) => e.status === 'PAID' && (e.paidAt || e.date) < dateRange.start).forEach((e:any) => bal -= e.amount);
            return bal;
        };

        const initialBalance = getPreviousBalance();
        ...
        rows.push([
            ...
            '-', 
            '-', 
            <span className="font-bold">{formatMoney(initialBalance)}</span>
        ]);
        ...
        rows.push(['TOTAL', 'Resumo do Período', '', formatMoney(totalIn), formatMoney(totalOut), formatMoney(runningBalance)]);

        return {
            headers: ['Data', 'Descrição', 'Categoria', 'Entradas', 'Saídas', 'Saldo Acumulado'],
            rows,
            summary: [
                { label: 'Saldo Anterior', value: initialBalance, color: '#64748b' },
                { label: 'Entradas Período', value: totalIn, color: '#16a34a' },
                { label: 'Saídas Período', value: totalOut, color: '#dc2626' },
                { label: 'Saldo Final', value: runningBalance, color: '#2563eb', subtext: 'Disponível em Caixa/Banco' },
            ],
            footerNote: "O Saldo Anterior considera toda a movimentação histórica registrada antes da data de início do filtro.",
            generatedAt: new Date().toLocaleString()
        };
    }

    // --------------------------------------------------------------------------------
    // 2. CASH BY ROUTE (RENTABILIDADE POR LINHA) - IMPLEMENTED
    // --------------------------------------------------------------------------------
    if (selectedReportId === 'CASH_BY_ROUTE') {
        const lineStats: Record<string, { name: string, pax: number, rev: number, exp: number, trips: number }> = {};
        const lines = data.lines;
        const routes = data.routes;
        
        // RE-IMPLEMENTING LOGIC WITH CORRECT DATA ACCESS
        const routeDefs = storage.getRoutes(); // Direct access for report generation
        
        data.routes.filter((r:any) => filterDate(r.date)).forEach((r:any) => {
            const routeDef = routeDefs.find((rd:any) => rd.id === r.routeId);
            const line = lines.find((l:any) => l.id === routeDef?.lineId);
            const lineName = line?.name || 'Linha Não Identificada';
            const lineId = line?.id || 'unknown';

            if (!lineStats[lineId]) {
                lineStats[lineId] = { name: lineName, pax: 0, rev: 0, exp: 0, trips: 0 };
            }
            
            lineStats[lineId].pax += r.passengers;
            lineStats[lineId].rev += r.revenueInformed;
            lineStats[lineId].exp += r.cashExpenses;
            lineStats[lineId].trips += 1;
        });

        const rows = Object.values(lineStats).map(s => [
            s.name,
            s.trips,
            s.pax,
            formatMoney(s.rev),
            formatMoney(s.exp),
            <span className={s.rev - s.exp > 0 ? 'text-green-700 font-bold' : 'text-red-600 font-bold'}>{formatMoney(s.rev - s.exp)}</span>
        ]);

        const totalRev = Object.values(lineStats).reduce((a,b) => a + b.rev, 0);
    ...
    // --------------------------------------------------------------------------------
    // 3. DRE SYNTHETIC (REGIME DE COMPETÊNCIA)
    // --------------------------------------------------------------------------------
    if (selectedReportId === 'DRE_SYNTHETIC') {
        ...
        // Correct Agency Commission Logic (Use stored snapshot or current rule)
        let costAgencyComm = 0;
        let costAgencyExp = 0;

        data.agencies.filter((a: any) => filterDate(a.date)).forEach((a: any) => {
             // Expenses inside agency box
             const exps = (a.expenses || []).reduce((s:number, e:any) => s+e.amount, 0);
             costAgencyExp += exps;

             // Commission
             let pct = a.commissionPctSnapshot;
             if (pct === undefined) {
                 const rule = data.commissions.find((c:any) => c.targetType === 'AGENCY' && c.targetId === a.agencyId && c.active);
                 pct = rule ? rule.percentage : 0;
             }
             const commVal = a.valueInformed * (pct / 100);
             costAgencyComm += commVal;
        });
        
        const totalVariable = costFuel + costRouteExp + costAgencyComm + costAgencyExp;
        ...
        const rows = [
            ...
            ['   Comissões s/ Vendas', formatMoney(costAgencyComm), totalRev ? ((costAgencyComm/totalRev)*100).toFixed(1)+'%' : '0%'],
            ['   Taxas de Agência', formatMoney(costAgencyExp), totalRev ? ((costAgencyExp/totalRev)*100).toFixed(1)+'%' : '0%'],
            ['', '', ''],
        ...
        return {
            ...
            footerNote: "Regime de Competência. As comissões são calculadas com base nas regras cadastradas ou históricas, e não pelo fluxo de caixa.",
            generatedAt: new Date().toLocaleString()
        };
    }

    // --------------------------------------------------------------------------------
    // 4. FUEL EFFICIENCY (CORRIGIDO)
    // --------------------------------------------------------------------------------
    if (selectedReportId === 'FUEL_EFFICIENCY') {
        ...
        const allEntries = [...data.fuel].sort((a:any,b:any) => a.date.localeCompare(b.date) || (a.mileage - b.mileage));
        const vehicles = data.vehicles;
        
        vehicles.forEach((v: Vehicle) => {
            const vEntriesInRange = allEntries.filter((e:any) => e.vehicleId === v.id && filterDate(e.date));
            
            if (vEntriesInRange.length > 0) {
                let totalLiters = 0;
                let totalAmount = 0;
                let totalDist = 0;

                const firstEntryInRange = vEntriesInRange[0];
                const prevEntry = allEntries.filter(e => e.vehicleId === v.id && e.date < firstEntryInRange.date).pop();
                const startKm = prevEntry ? prevEntry.mileage : (v.initialMileage || 0);
                const endKm = vEntriesInRange[vEntriesInRange.length - 1].mileage;
                ...
                vEntriesInRange.forEach(e => {
                    totalLiters += e.liters;
                    totalAmount += e.amount;
                });

                vehicleStats[v.id] = { liters: totalLiters, amount: totalAmount, dist: totalDist, entries: vEntriesInRange.length };
            }
        });
        ...
        Object.entries(vehicleStats).forEach(([vid, stat]) => {
            ...
            rows.push([
                ...
                isAnomaly ? <span className="text-slate-400 italic" title="Valor irreal. Verifique odômetro.">-</span> : <span className={kmL < 2.5 ? 'text-red-600 font-bold' : 'text-green-700 font-bold'}>{kmL.toFixed(2)} km/L</span>,
                isAnomaly ? '-' : 'R$ ' + costKm.toFixed(2)
            ]);
        });
        ...
        return {
            ...
            footerNote: "A distância é calculada subtraindo a KM do último abastecimento no período pela KM do abastecimento IMEDIATAMENTE ANTERIOR (ou KM inicial do cadastro). Valores irreais (>15km/L) são ocultados como erro de digitação.",
            generatedAt: new Date().toLocaleString()
        };
    }
    ...
    if (selectedReportId === 'PAYABLES_OPEN' || selectedReportId === 'PAYABLES_PAID') {
        ...
        const items = data.expenses.filter((e: GeneralExpense) => {
            if (isPaidMode) {
                ...
                return e.status === 'PENDING' && e.date <= dateRange.end;
            }
    ...
    // --------------------------------------------------------------------------------
    // 6. AUDIT CLOSING (QUEBRAS DE CAIXA) - IMPLEMENTED
    // --------------------------------------------------------------------------------
    if (selectedReportId === 'AUDIT_CLOSING') {
        const closes = data.closes.filter((c: DailyClose) => filterDate(c.date)).sort((a:DailyClose,b:DailyClose) => a.date.localeCompare(b.date));
        
        const rows = closes.map((c: DailyClose) => [
            formatDateDisplay(c.date),
            formatMoney(c.totalRouteRevenue + c.totalAgencyRevenue + (c.totalTourismRevenue || 0)),
            formatMoney(c.totalExpenses + (c.totalAgencyExpenses||0) + (c.totalTourismExpenses||0) + (c.totalGeneralExpenses||0) + c.totalCommissions),
            formatMoney(c.netResult),
            <span className={c.totalDiff < -0.1 ? 'text-red-600 font-bold' : (c.totalDiff > 0.1 ? 'text-blue-600 font-bold' : 'text-slate-400')}>{formatMoney(c.totalDiff)}</span>,
            new Date(c.closedAt).toLocaleTimeString()
        ]);

        const totalDiff = closes.reduce((a:number, b:DailyClose) => a + b.totalDiff, 0);
        const totalNet = closes.reduce((a:number, b:DailyClose) => a + b.netResult, 0);

        rows.push(['TOTAL', '-', '-', formatMoney(totalNet), formatMoney(totalDiff), '-']);

        return {
            headers: ['Data', 'Receita Bruta', 'Despesas/Saídas', 'Saldo Final (Caixa)', 'Diferença (Quebra)', 'Hora Fechamento'],
            rows,
            summary: [
                { label: 'Dias Fechados', value: closes.length },
                { label: 'Saldo Acumulado', value: totalNet, color: '#16a34a' },
                { label: 'Total de Quebras', value: totalDiff, color: totalDiff < 0 ? '#dc2626' : '#2563eb' }
            ],
            footerNote: "Valores positivos na coluna 'Diferença' indicam Sobra de Caixa. Valores negativos indicam Falta de Caixa (Prejuízo).",
            generatedAt: new Date().toLocaleString()
        };
    }
